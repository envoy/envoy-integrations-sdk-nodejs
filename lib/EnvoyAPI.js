...
class EnvoyAPI {
  ...
  /**
   * Legacy endpoint, use with caution. 
  ...
  async getEntry(entryId) {
    var options = {
      method: 'GET',
      url: this.newUrl + `/entries/${entryId}`,
      headers: {
        Authorization: 'Bearer ' + this.token
      },
      json: true,
   ...
   * 
   * @param { {entries} } entries Optional object containing one or more of the below params:
   * 
   * locationIds - array of strings
   * signedInAtAfter - a string in date-time format
   * signedInAtBefore - a string in date-time format
   * email - string
   * page - number
   * perPage - number 
   * sort - string 
   ...
   * Updates an entry.
   * 
   * @param { {entry} } entry Object containing entry id and fields to update. See below for params:
   * id - string | number
   * fullName - string
   * email - string
   * customFields - array of objects 
   *   Example: [ {field: customName, value: 'customValue' } ] 
   * @returns {Promise<EnvoyObject>}
   */
  async updateEntry({ id, fullName, email, customFields }) {
    var options = {
      method: 'POST',
      url: this.newUrl + `/entries/${id}`,
      headers: {
        Authorization: 'Bearer ' + this.token
      },
      body: {
  ...
  /** 
    ...
    * @param {{}} entry Object containing body params. See below for possible values:
    * locationId - string | number
    * flowId - string | number
    * fullName - string
    * email - string
    * customField - array of objects 
    *   Objects have format { field: {string}, value: {string} }
    *   Example: [{field: customFieldName, value: customFieldValue}]
    * @returns {Promise<EnvoyObject>}
    */
  async entryCreate(entry) {
    var options = {
      method: 'POST',
      url: this.newUrl + `/entries`,
      headers: {
  ...
  /**
   * Deletes a work schedule item. Returns deleted item on success.
   * @param {{}} workSchedule object.
   * @returns {Promise<EnvoyObject>}
   */
  ...
  /**
   * Possibly deprecated? Endpoint no longer accessible and similar function is available at EnvoyPluginStorage
   * Gets the plugin install's config.
   *
   * @param {number} installId
   * @returns {Promise<{}>}
   */
  async getPluginInstallConfig(installId, pluginAccessToken = null) {
  ...
  static async getAccessTokenFromAuthCode(authCode) {
    const ENVOY_AUTH_URL = process.env.ENVOY_AUTH_URL || 'https://app.envoy.com/a/auth/v0/token';
