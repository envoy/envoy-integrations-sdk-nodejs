...
class EnvoyAPI {
  ...
  constructor(token, xEnvoyContext = {}) {
    ...
    // Validation URL OAuth redirects to on successfull login.
    this.envoyValidationUrl = 'https://envoy-integrations-auth.herokuapp.com/redirect-login';
  ...
  /**
   * Starts OAuth flow by opening a window in a client's default browser to Envoy Auth url.
   * On successful login users will be redirected to a validation url managed by Envoy Or to a specific redirect url if provided in args.
   * If using Envoy validation url, this method will start a temporary local express instance from which Envoy will send an encrypted refresh and bearer token at port 4001.
   *  
   * @param { array } scopes - Array of strings containing requested scopes for this user.
   * @param {*} validationURL - Optional validation url if user wants to use their own validation endpoint.
   * @returns 
   */
  static async externalAuthLogin(scopes, validationURL) {
    let baseURL = this.envoyAuthUrl || process.env.ENVOY_AUTH_URL;
    let clientID = `&client_id=${process.env.ENVOY_CLIENT_ID}`;
    let redirectURI = `&redirect_uri=${validationURL || this.envoyValidationUrl || process.env.AUTH_VAL_URL}`;
    let scope = `&scope=` + scopes.join('+');

    let redirectURL = baseURL + clientID + redirectURI + scope;
    // await open(redirectURL);

    // If validation url is provided in args, assume user will handle tokens themselves on their endpoint.
    if (validationURL) return;

    const app = express();
    app.use(express.json());
    app.post('/envoy-get-tokens', async (req, res) => {
      console.log(req.body);
      console.log('Closing Server on port 4104');
      listener.close();
      res.send(); 
    });

    const listener = app.listen(4104 || 0, () => {
      console.log(`EnvoySDK listening on port: ${listener.address().port}`);
    })
  }
}
