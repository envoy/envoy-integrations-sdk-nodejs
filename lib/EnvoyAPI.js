...
const promisifiedRequest = promisify(cypressRequest);

// Add defaults method to maintain compatibility with request-promise-native
promisifiedRequest.defaults = function(defaultOptions) {
  const requestWithDefaults = cypressRequest.defaults(defaultOptions);
  const promisifiedWithDefaults = promisify(requestWithDefaults);
