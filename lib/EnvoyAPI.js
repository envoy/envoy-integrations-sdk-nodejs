...
class EnvoyAPI {
  ...
  async companyRoles(userId) {
    ...
    const body = await this.request({
      url: `/api/v2/users/${userId}/relationships/company-roles`,
    });
  ...
  async employee(companyRoleId) {
    ...
    const body = await this.request({
      url: `/api/v2/company-roles/${companyRoleId}/employee`,
    });
  ...
  static loginAsUser(email, password, id = process.env.ENVOY_CLIENT_ID, secret = process.env.ENVOY_CLIENT_SECRET) {
    const baseUrl = process.env.ENVOY_BASE_URL || 'https://app.envoy.com';
    return request({
      auth: {
        user: id,
