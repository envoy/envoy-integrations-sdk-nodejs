...
export default class EnvoyPluginStorage {
  ...
  /**
   * Updates a single {@link EnvoyStorageItem} from storage.
   *
   * Wrapper for single pipeline update.
   */
  update<Value = unknown>(key: string, value: Value) {
    return this.pipeline().update(key, value).executeSingle<EnvoyStorageItem<Value>>();
  }
